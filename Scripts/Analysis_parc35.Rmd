...
cat("Variable dcode has statistical significance for ",sum(p_val[,"dcode"]<0.05),"regions\n")
cat("Variable dcode_age has statistical significance for ",sum(p_val[,"dcode_age"]<0.05),"regions\n")
```
...
cat("Variable dcode has statistical significance for ", sum(p_val_FDR[,"dcode"]<0.05),"regions\n")
cat("Variable dcode_age has statistical significance for ",sum(p_val_FDR[,"dcode_age"]<0.05),"regions")
```
...
Z <- Zs_match %>%
  ...
  summarise(mul = prod(dev)) %>% # Multiplicamos los dev de todos los tp de cada subject por cada región
...
Z <- Zs_match %>%
  ...
  group_by(region,subID) %>%
  summarise(mul = prod(cond)) %>% # Multiplicamos todos los tp de cada subject para cada región
...
for (tp in 1:3){ # for each timepoint
  ...
  Z_pat <- Zs_match %>%
    ...
    group_by(subID) %>%
    summarise(nreg_dev_infra    = sum(deviant_infra==1),   # At least one region deviated
...
getStatistics(Zs_match,  "lC_match_parc35")
getStatistics(Zs_NOmatch,"lC_NOmatch_parc35")
```
...
img1 <- rasterGrob(as.raster(readPNG("/data_J/Results/Plots/prevalence_wscores_1_lC_match_parc35.png")), interpolate = FALSE)
img2 <- rasterGrob(as.raster(readPNG("/data_J/Results/Plots/prevalence_wscores_2_lC_match_parc35.png")), interpolate = FALSE)
img3 <- rasterGrob(as.raster(readPNG("/data_J/Results/Plots/prevalence_wscores_3_lC_match_parc35.png")), interpolate = FALSE)
...
img1 <- rasterGrob(as.raster(readPNG("/data_J/Results/Plots/prevalence_wscores_1_lC_NOmatch_parc35.png")), interpolate = FALSE)
img2 <- rasterGrob(as.raster(readPNG("/data_J/Results/Plots/prevalence_wscores_2_lC_NOmatch_parc35.png")), interpolate = FALSE)
img3 <- rasterGrob(as.raster(readPNG("/data_J/Results/Plots/prevalence_wscores_3_lC_NOmatch_parc35.png")), interpolate = FALSE)
...
tp <- 1
lab <- "lC_matched_parc35"
paste0("/data_J/results/Plots/violin_tp",tp,"_",lab,".png")
```
